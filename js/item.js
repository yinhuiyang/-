...
$('.fit-ul li').click(function(){
	$(this).addClass('active-three').siblings().removeClass('active-three')
	var i=$(this).index();
	$('.cont-ul-one').eq(i).addClass('active-four').siblings().removeClass('active-four')
})
...
//	  	
//	 var timer=new Date();
//	 var value=$('.amont input').val();
//	 window.localStorage.setItem(timer,value) 
//
//	  })
...
//	console.log(t)
	if(t >= 1136) {
		$('.tab-main').css('width','990px').css('position','fixed').css("top", '0').css('background','#F7F7F7')
	} else {	
		$('.tab-main').css('position','relative')
	}
	if(t >= 1450) {
		$('.detail-head').css('position','fixed').css("top", '40px')
	} else {	
		$('.detail-head').css('position','relative').css("top", '0px')
	}
...
$('#store-selector').hover(function(){
	$('#store-selector .text').css('border','1px solid #ccc').css('border-bottom','none')
},function(){
	$('#store-selector .text').css('border','1px solid #fff')
})
